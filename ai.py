...
MESSAGE_CACHE_FILE = os.path.join(CONVERSATION_FOLDER, "message_cache.json")  # New: Cache for message responses
...
feature_proposals = []
clone_data = {}
message_cache = {}  # New: In-memory cache for message-response pairs
...
def run_async(coro):
    """Run an async coroutine synchronously in the dedicated event loop."""
...
def huggingface_generate(prompt, retries=3):
    return run_async(huggingface_generate_async(prompt, retries))
...
def challenge_command(message):
    ...
    if level < len(CHALLENGE_LEVELS):
        challenge = CHALLENGE_LEVELS[level]
        bot.reply_to(message, f"🎯 *{nickname}*, yeh tera challenge: {challenge['task']}\n*Reward*: {challenge['reward']} ♥", 
                     parse_mode="Markdown", reply_markup=keyboard)
    else:
        bot.reply_to(message, f"🏆 *{nickname}*, tune saare challenges khatam kar diye! ★", parse_mode="Markdown", reply_markup=keyboard)
...
def feedback_command(message):
    ...
    if feedback:
        ...
        if user_id != OWNER_ID:
            bot.send_message(OWNER_ID, f"🔔 *Feedback* from *{nickname}*: {feedback} ★", parse_mode="Markdown")
    else:
        keyboard = InlineKeyboardMarkup()
...
def approve_feature_command(message):
    ...
    try:
        ...
        if 0 <= index < len(feature_proposals):
            ...
            save_user_data()
            bot.reply_to(message, f"✅ *{nickname}*, feature approved: {proposal['feature']} by *{proposal['nickname']}*! 🌟", 
                         parse_mode="Markdown")
            if proposal["user_id"] != OWNER_ID:
                bot.send_message(proposal["user_id"], f"🎉 Tera feature *{proposal['feature']}* approved ho gaya! ✨", 
                                parse_mode="Markdown")
        else:
...
def reject_feature_command(message):
    ...
    try:
        ...
        if 0 <= index < len(feature_proposals):
            ...
            save_user_data()
            bot.reply_to(message, f"❌ *{nickname}*, feature rejected: {proposal['feature']} by *{proposal['nickname']}*! 😔", 
                         parse_mode="Markdown")
            if proposal["user_id"] != OWNER_ID:
                bot.send_message(proposal["user_id"], f"😔 Tera feature *{proposal['feature']}* reject ho gaya. Try again! ✨", 
                                parse_mode="Markdown")
        else:
            bot.reply_to(message, f"🤔 *{nickname}*, invalid proposal ID! 😅", parse_mode="Markdown")
    except ValueError:
...
def handle_text(message):
    ...
    # New: Check for simple text messages (no commands)
    if not text.startswith('/'):
        # Check message cache for existing response
        ...
        if message_key in message_cache:
            ...
            # Generate new response using Hugging Face API
            ...
            # Save conversation and cache response
    ...
    # Existing logic for keyword-based or command-based messages
...
def main():
    ...
    load_user_data()
    load_clone_data()
    load_message_cache()  # New: Load message cache at startup
