...
def is_owner(user_id, username):
    ...
    if user_id == OWNER_ID:
        if username != OWNER_USERNAME:
            ...
            logging.info(f"Owner username updated to {username}")
        return True
    return False

# 🌐 API Functions
async def huggingface_generate(prompt):
...
async def elevenlabs_voice(text):
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
...
async def get_pexels_image(query="romantic couple"):
    async with aiohttp.ClientSession() as session:
        try:
...
async def get_giphy_gif(query="love"):
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
