...
def run_schedule():
    while True:  # Run indefinitely, handle status safely
        if not check_original_status():
            time.sleep(1)  # Avoid busy-waiting when bot is offline
            continue
        try:
