...
def save_user_data():
    with file_lock:
        # Convert user_data to a JSON-serializable format
        ...
        if os.path.exists(USER_DATA_FILE):
            with open(USER_DATA_FILE, "r", encoding="utf-8") as f:
                ...
                for user_id, info in loaded_data.items():
                    # Convert question_reset_time back to datetime
        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(retries):
                try:
                    ...
                    asyncio.sleep(1)
    return loop.run_until_complete(_voice())
        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(retries):
                try:
                    ...
                    asyncio.sleep(1)
    return loop.run_until_complete(_image())
        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(retries):
                try:
                    ...
                    asyncio.sleep(1)
    return loop.run_until_complete(_gif())
        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(retries):
                try:
                    ...
                    asyncio.sleep(1)
    return loop.run_until_complete(_stock())
        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(retries):
                try:
                    ...
                    asyncio.sleep(1)
    return loop.run_until_complete(_news())
