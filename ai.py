...
def process_auto_response(user_id, nickname, response, mood, is_owner_user, response_context, response_type, original_text=None):
    try:
        if is_owner_user:
            ...
            response = random.choice(FINANCIAL_COMPLIMENTS).format(name=nickname) + "\n" + response
        else:
            response = random.choice(COMPLIMENTS).format(name=nickname) + "\n" + response

        if response_type == "text":
            bot.send_message(user_id, response, parse_mode="Markdown")
        elif response_type == "voice":
            voice_file = generate_voice_message(response)
            ...
            image_file = get_pexels_image()
            if image_file and os.path.exists(image_file):
                with open(image_file, 'rb') as f:
                    bot.send_photo(user_id, f, caption=response)
            else:
                bot.send_message(user_id, response, parse_mode="Markdown")
        elif response_type == "gif":
            gif_file = get_giphy_gif()
            if gif_file and os.path.exists(gif_file):
                with open(gif_file, 'rb') as f:
                    bot.send_animation(user_id, f, caption=response)
            else:
                bot.send_message(user_id, response, parse_mode="Markdown")
        elif response_type == "sticker":
            sticker_id = get_sticker()
            if sticker_id and not sticker_id.startswith("😅"):
                bot.send_sticker(user_id, sticker_id)
                bot.send_message(user_id, response, parse_mode="Markdown")
            else:
                bot.send_message(user_id, response, parse_mode="Markdown")
        elif response_type == "video":
            video_data = {"text": response, "template": "basic"}
            project_id = create_json2video(video_data)
            if isinstance(project_id, str):
                status = get_json2video_status(project_id)
                if status == "completed":
                    video_file = save_media_to_folder(requests.get(f"{JSON2VIDEO_BASE_URL}/{project_id}/download").content, "video")
                    if video_file and os.path.exists(video_file):
                        with open(video_file, 'rb') as f:
                            bot.send_video(user_id, f, caption=response)
                    else:
                        bot.send_message(user_id, response, parse_mode="Markdown")
                else:
                    bot.send_message(user_id, response, parse_mode="Markdown")
        ...
        if original_text:
            save_chat(user_id, original_text, response)
    except Exception as e:
        print(f"❌ Error in process_auto_response for {user_id}: {e}")
        fallback = f"😅 *{nickname}* jaan, thodi si dikkat! Try again! 😘"
        bot.send_message(user_id, fallback, parse_mode="Markdown")
        if original_text:
            save_chat(user_id, original_text, fallback)
...
def chat_with_ai(message):
    ...
    try:
        ...
        save_chat(user_id, message.text, fallback  fallback)
...
def auto_reply():
    ...
    while True:
        try:
            for user_id, nickname in active_chats.items():
                if user_id not in last_reply_time or (datetime.now(IST) - last_reply_time.get(user_id, datetime.now(IST))).total_seconds() > 3600:
                    mood = random.choice(list(mood_keywords.keys()))
                    response = f"😘 *{nickname}* jaan, Rani misses you! Kya chal raha hai? 💕"
                    response = translate_text(response, user_data[user_id]["language"])
                    bot.send_message(user_id, response, parse_mode="Markdown")
                    last_reply_time[user_id] = datetime.now(IST)
        except Exception as e:
...
if __name__ == "__main__":
    print(f"🤖 {ORIGINAL_NAME} is starting...")
