...
async def periodic_promo(app):
    while check_original_status():
        try:
            ...
            logger.error(f"Periodic promo error: {e}")
...
async def main():
    ...
    try:
        ...
        tasks.append(asyncio.create_task(periodic_sync(app)))
        tasks.append(asyncio.create_task(periodic_challenge(app)))
        tasks.append(asyncio.create_task(periodic_promo(app)))
        
        # Run polling to handle updates
        await app.run_polling(allowed_updates=Update.ALL_TYPES, stop_signals=None)
    
    except Exception as e:
        logger.error(f"Bot stopped unexpectedly: {e}", exc_info=True)
        raise  # Re-raise to ensure proper handling by asyncio.run()
        ...
        try:
            ...
            # Stop and shutdown the application if still running
            if app.running:
                await app.stop()
            await app.shutdown()
